import { Option, type MapOptions } from '../../util/option';
...
export class ArgumentDescriptor<T extends unknown[] = []> {
	...
	private resolveParsers(parsers: Parsers) {
		return Array.isArray(parsers) ? parsers : [parsers];
	}
	...
	public compile(args: string[] = this.defaultArgs) {
		const results = Option.nones(this.steps.length) as MapOptions<T>;
